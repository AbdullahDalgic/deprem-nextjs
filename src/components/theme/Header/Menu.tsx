              ...
              {menuItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    href={item.href}
                    prefetch={true}
                    className={`group relative px-1.5 lg:px-2.5 xl:px-3 py-1.5 lg:py-2 rounded-lg transition-all duration-200 flex items-center gap-0.5 lg:gap-1.5 xl:gap-2 font-medium text-xs lg:text-sm whitespace-nowrap ${active
                  ...
                  >
                    <Icon className={`w-3 h-3 lg:w-3.5 lg:h-3.5 xl:w-4 xl:h-4 flex-shrink-0 hidden lg:block transition-opacity ${active ? "opacity-100" : "opacity-70 group-hover:opacity-100"
                      }`} />
                    {item.mobileLabel ? (
                      <>
                    ...
                    <span className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-0.5 bg-primary transition-all duration-200 ${active ? "w-full" : "w-0 group-hover:w-3/4"
                      }`}></span>
                  </Link>
                );
              })}
