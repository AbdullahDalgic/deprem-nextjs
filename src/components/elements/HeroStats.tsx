...
export default function HeroStats({ earthquakes }: HeroStatsProps) {
  ...
  // En büyük deprem
  const largestEarthquake = earthquakes.length > 0
    ? earthquakes.reduce((prev, current) =>
        current.magnitude > prev.magnitude ? current : prev
      )
    : null;

  // En büyük deprem linki
  const largestEarthquakeLink = largestEarthquake
    ? generateEarthquakeLink(largestEarthquake)
    : null;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-8">
      {stats.map((stat, index) => {
        ...
        const commonClassName = "group relative rounded-2xl p-6 md:p-8 bg-white/10 dark:bg-white/5 backdrop-blur-md border border-white/20 dark:border-white/10 shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105 hover:bg-white/15 dark:hover:bg-white/10 overflow-hidden flex items-start";
